...
class SandboxWindow(QMainWindow):
    ...
    def setup_widgets(self):
        ...
        self.single_die = DieWidget(
            sides=6, bg_color=QColor("gold"), fg_color=QColor("darkblue")
        )
    ...
    def roll_single(self):
        ...
        QTimer.singleShot(
            1000,
            lambda: self.single_die.stop_roll(
                rnd_val := GameDieFactory.random_die(
                    DieType.from_sides(self.single_die.sides)
                ).roll()
            ),
        )
